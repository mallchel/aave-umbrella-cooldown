...
func (r *Repository) ListDailyFlowPoints(ctx context.Context) ([]DailyFlowPoint, error) {
	query := `
		SELECT
			...
			COALESCE(SUM(CASE WHEN event_type = 'request' THEN amount_usdt::numeric ELSE 0 END), 0)::double precision AS requested,
			COALESCE(COUNT(CASE WHEN event_type = 'request' THEN amount_usdt::numeric ELSE 0 END), 0)::double precision AS request_count,
			COALESCE(SUM(CASE WHEN event_type = 'withdraw' THEN amount_usdt::numeric ELSE 0 END), 0)::double precision AS withdrawn
